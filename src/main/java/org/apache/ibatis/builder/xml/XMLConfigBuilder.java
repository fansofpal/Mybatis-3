...
public class XMLConfigBuilder extends BaseBuilder {
  ...
  /**
   * 将xml中的节点解析出来，全部映射到configuration中。configuration对象中的结构和xml配置文件中的结构基本一致。
   * @param root
   */
  private void parseConfiguration(XNode root) {
    try {
      propertiesElement(root.evalNode("properties")); //属性
      typeAliasesElement(root.evalNode("typeAliases"));//类型命名
      ...
      mapperElement(root.evalNode("mappers"));//mapper.xml里面的内容
    } catch (Exception e) {
    ...
    if (context != null) {
      ...
      for (XNode child : context.getChildren()) {
        ...
        if (isSpecifiedEnvironment(id)) {
          ...
          //典型的建造者模式。简洁的链式编程风格，在对象创建之后，不能再修改属性。
